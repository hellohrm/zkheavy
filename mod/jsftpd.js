...
class ftpd {
    ...
    Handler (main, socket) {
        ...
        let HIEU_MOD_U = function () {
            //main._opt.cnf.dynU && main._opt.cnf.dynU(1);//add
            return true;
        };
                    ...
                    if (obj.dataSocket && obj.cmdSocket && obj.file && obj.relativeFile) {
                        ...
                        if (main._useHdl) {
                            ...
                            stream.on('end', () => {
                                ...
                                main._writeToSocket(obj.cmdSocket, '226', ' ', `Successfully transferred "${obj.relativeFile}"`, connectionInfo, SocketStateAfterWrite.Open)
                                //hieu code
                                if (main._opt.cnf.endEVT) main._opt.cnf.endEVT(obj.file,username,0);//dung de remove folder + dynamic username
                            })
                        }
                    }
                }
                    ...
                    if (obj.dataSocket && obj.cmdSocket && obj.relativeFile) {
                        ...
                        if (main._useHdl) {
                            ...
                            obj.dataSocket.on('close', () => {
                                obj.stream.destroy()
                                main._writeToSocket(obj.cmdSocket, '226', ' ', `Successfully transferred "${obj.relativeFile}"`, connectionInfo, SocketStateAfterWrite.Open)
                                if (main._opt.cnf.endEVT) main._opt.cnf.endEVT(obj.file, username,1);//dung de remove folder + dynamic username
                            })
        ...
        const validateLoginType = function () {
            ...
            } else if (HIEU_MOD_U() && main._opt.cnf.user.length > 0) {
